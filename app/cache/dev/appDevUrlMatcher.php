...
class appDevUrlMatcher extends Symfony\Bundle\FrameworkBundle\Routing\RedirectableUrlMatcher
{
    ...
    public function match($pathinfo)
    {
        ...
        if (preg_match('#^/(?P<id>\\d+)/edit$#s', $pathinfo, $matches)) {
            if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                $allow = array_merge($allow, array('GET', 'HEAD'));
                goto not_BloggerBlogBundle_edit;
            }

            return $this->mergeDefaults(array_replace($matches, array('_route' => 'BloggerBlogBundle_edit')), array (  '_controller' => 'Blogger\\BlogBundle\\Controller\\BlogController::editAction',));
        }
        not_BloggerBlogBundle_edit:

        // BloggerBlogBundle_new
        if (preg_match('#^/(?P<id>\\d+)$#s', $pathinfo, $matches)) {
            if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                $allow = array_merge($allow, array('GET', 'HEAD'));
                goto not_BloggerBlogBundle_new;
            }

            return $this->mergeDefaults(array_replace($matches, array('_route' => 'BloggerBlogBundle_new')), array (  '_controller' => 'Blogger\\BlogBundle\\Controller\\BlogController::newAction',));
        }
        not_BloggerBlogBundle_new:

        // BloggerBlogBundle_delete
        if (0 === strpos($pathinfo, '/delete') && preg_match('#^/delete/(?P<id>\\d+)$#s', $pathinfo, $matches)) {
            if ($this->context->getMethod() != 'DELETE') {
                $allow[] = 'DELETE';
                goto not_BloggerBlogBundle_delete;
            }

            return $this->mergeDefaults(array_replace($matches, array('_route' => 'BloggerBlogBundle_delete')), array (  '_controller' => 'Blogger\\BlogBundle\\Controller\\BlogController::deleteAction',));
        }
        not_BloggerBlogBundle_delete:
